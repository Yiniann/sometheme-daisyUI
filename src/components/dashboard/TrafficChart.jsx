...
const TrafficChart = () => {
  ...
  const chartData = useMemo(() => {
    ...
    return {
      series: [
        { name: "下载用量", data: reversedLogs.map((log) => log.d) },
        { name: "上传用量", data: reversedLogs.map((log) => log.u) },
      ...
      options: {
        ...
        xaxis: {
          ...
          labels: { style: { colors: themeColors.labelColor, fontWeight: 500 } },
        },
        yaxis: {
          labels: {
            show: false,
        ...
        legend: { position: "bottom", horizontalAlign: "center", labels: { colors: themeColors.labelColor } },
        dataLabels: { enabled: false },
        markers: { size: 0 },
        colors: themeColors.series,
        stroke: { curve: "smooth", width: [2, 2, 2], dashArray: [3, 3, 0] },
        grid: { show: false },
        tooltip: {
          theme: false,
  ...
  return (
    <StatusMessage loading={loading} error={error} loadingText="流量数据加载中..." errorText="加载流量数据失败">
      {trafficLogs && trafficLogs.length > 0 ? (
        ...
        <div className="text-center text-sm text-base-content/60 py-8">暂无流量数据</div>
      )}
    </StatusMessage>
  );
};
